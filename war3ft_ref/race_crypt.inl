...
public _CL_ULT_LocustEffect( parm[] )
{
	...
	if ( !is_user_alive( iVictim ) || !p_data_b[iVictim][PB_ISCONNECTED] )
	{
		...
		client_print( iAttacker, print_chat, "%s Victim is hidden for Locust Swarm.", GAME_NAME );
	...
	else
	{
		...
		client_print( iAttacker, print_chat, "%s Locust Swarm hit %s with %d Damage.", GAME_NAME, szName, iDamage );
	}
}
