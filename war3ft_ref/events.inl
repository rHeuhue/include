...
public grenade_throw( index, greindex, wId )
{
	...
	if ( ITEM_Has( index, ITEM_GLOVES ) > ITEM_NONE && SHARED_IsGrenade( wId ) )
	{
	...
	if ( iSkillLevel > 0 && OR_CriticalGrenadeAllowed( index ) && SHARED_IsGrenade( wId ) && greindex)
	{
		
		new iWidth = 3 * iSkillLevel;

		Create_TE_BEAMFOLLOW( greindex, g_iSprites[SPR_NEW_TRAIL], 20, iWidth, 255, 35, 35, 195 );
	...
	iSkillLevel = SM_GetSkillLevel( index, SKILL_BITE);
	
	if (SHARED_IsSmokeGrenade(wId) && greindex)
	{
		// Give another just if he has frostbite skill 
		if(iSkillLevel > 0)
		{
			CM_Frostbite_Begin( index );
		}
		
		new iWidth = 3 * 3;

		Create_TE_BEAMFOLLOW(greindex, g_iSprites[SPR_NEW_TRAIL], 20, iWidth, 0, 100, 200, 195);
		
		if(iSkillLevel > 0)
		{
			emit_sound(index, CHAN_STATIC, g_szSounds[SOUND_ICE], 1.0, ATTN_NORM, 0, PITCH_NORM);
		}
	}
...
public EVENT_TakeDamage( iVictim, iWeapon, iAttacker, Float:f_Damage, damagetype )
{
	...
	{
		static iSkillLevel;
		iSkillLevel = SM_GetSkillLevel( iVictim, SKILL_HIDES );
		if ( iSkillLevel > 0 )
		{   
			if ( random_float( 0.0, 1.0 ) <= p_hides[iSkillLevel-1] )
				{
					...
					{
						client_print( iVictim, print_chat, "%s You have blocked a grenade.", GAME_NAME );
...
public EVENT_PlayerInitialSpawn( id )
{
	...
	{
		client_print( id, print_chat, "%s WC3 Cheats are enabled on this server! Try typing /level10", GAME_NAME );

		p_data_b[id][PB_JUSTJOINED]			= false;
	}

	// Display the new Chameleon skills for the round
	if ( p_data[id][P_RACE] == 9 && get_pcvar_num( CVAR_wc3_cham_random ) && !Polymorph_Player[id] )
	{
		WC3_ShowRaceInfo( id );
	}
...
public TRIGGER_TraceLine( Float:v1[3], Float:v2[3], noMonsters, pentToSkip )
{
	...
	if (!pev_valid(iAttacker) || !pev_valid(iVictim))
		return FMRES_IGNORED;
