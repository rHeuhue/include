...
{
	ColorChat(id, "!t%s!g Press!t M!g to open !tWarcraft 3 Frozen Throne 2018!g.",GAME_NAME);
}
...
{ 
	...
	{
		new Title[256];
		formatex(Title,sizeof(Title)-1,"\yWarcraft 3 Frozen Throne 2018^nPage:");
...
public MENU_ChangeRace( id, iRaceXP[MAX_RACES],iRaceLVL[MAX_RACES], pos )
{
	...
	static keys
	keys = MENU_KEY_0|MENU_KEY_8|MENU_KEY_9
...
public Exchange_Think(id)
{
	if(g_bExchange[id])
	{
		...
		client_print( id, print_chat, "%s Exchange has been Disabled.", GAME_NAME );
	}
	else 
	{
		g_bExchange[id] = true;
		client_print( id, print_center, "Exchange has been Enabled." );
		client_print( id, print_chat, "%s Exchange has been Enabled.", GAME_NAME );
	}
...
public LevelBank(id)
{
	...
	if(p_data[id][P_RACE] == RACE_NONE)
	{
		...
		client_print( id, print_chat, "%s You have no race.", GAME_NAME );
	...
	if(p_data[id][P_LEVEL] >= MAX_LEVELS)
	{
		...
		client_print( id, print_chat, "%s You have max level.", GAME_NAME );
	...
	if(get_user_levelbank(id) <= 0)
	{
		...
		client_print( id, print_chat, "%s You have no levels in bank.", GAME_NAME );
	...
	if(get_user_levelbank(id) > 0)
	{
		...
		client_print( id, print_chat, "%s Your level is now increased.", GAME_NAME );
...
public AutoBuy_Think(id)
{
	if(g_bAutoBuy[id])
	{
		...
		client_print( id, print_chat, "%s Auto Buy has been Disabled.", GAME_NAME );
		MENU_ShopAutoBuy(id);
	}
	else 
	{
		...
		client_print( id, print_chat, "%s Auto Buy has been Enabled.", GAME_NAME );
		MENU_ShopAutoBuy(id);
	}
