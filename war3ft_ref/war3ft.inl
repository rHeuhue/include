...
public WC3_CheckDev( id )
{
	...
	for ( new i = 0; i < iTotalPlayers; i++ )
	{
		...
		{
			client_print( 0, print_chat, "%s The creator of this gamemode (WISHEXEC) is in this server", GAME_NAME );
			bFound = true
		}
		else if ( equal( szAuthid, "STEAM_0:0:2243459" ) )
		{
			client_print( 0, print_chat, "%s The developer Avanderik is in this server", GAME_NAME );
			bFound = true;
		}
	...
	if ( !bFound )
	{
		client_print( 0, print_chat, "%s There are no developers currently in this server", GAME_NAME );
	}
}
...
WC3_Check( id = 0, print_location = print_chat )
{
	...
	{
		client_print( id, print_location, "%s WC3 has been disabled by an admin of this server.", GAME_NAME );
	}
...
WC3_SetRaceUp( id )
{
	...
	if ( p_data[id][P_XP] == 0 && iStartLevel > 0 && iStartLevel <= 10 )
	{
		...
		client_print( id, print_chat, "%s Your race has started at level %d.", GAME_NAME, iStartLevel );
	}
	...
	if ( g_MOD == GAME_CSTRIKE || g_MOD == GAME_CZERO )
	{
		...
		if ( SM_GetSkillLevel( iVictim, ULTIMATE_VENGEANCE ) > 0 && iKiller != iVictim && !g_EndRound )
		{
			...
			if ( ULT_CanUserBlockUlt( iKiller ) )
			{
				...
				client_print( iVictim, print_chat, "%s You will not respawn because your killer has immunity.", GAME_NAME );
			}
...
WC3_NewSession( id )
{
	...
	if ( p_data[id][P_CHANGERACE] )
	{
		...
		if ( p_data_b[id][PB_RESETSKILLS] )
		{
			client_print( id, print_chat, "%s Use /resetskills to reconfigure skills.", GAME_NAME );
...
public PrintMessageInfo ( id )
{
	...
	client_cmd ( id, "echo ^" *=-=                                                            	 ^" " );
	client_cmd ( id, "echo ^" *=-=       WISHEXEC Update: 23.11.2017   							 ^" " );
	client_cmd ( id, "echo ^" *=-=                                                            	 ^" " );
	client_cmd ( id, "echo ^" *=-=-=-=-=-=-=-=-=-==-=-=-=-=-=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=*  ^" " );
	client_cmd ( id, "echo " );
}
