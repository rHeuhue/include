...
public _SHARED_Spawn( id )
{
	...
	if ( g_EndRound )
	{
		...
		{
			client_print( id, print_chat, "%s No respawn because the round is over, here is your money back.", GAME_NAME );
...
public _SHARED_Mole( parm[2] )
{
	...
	else
	{
		...
		if ( parm[1] == 2 )
		{
			...
			client_print( id, print_chat, "%s No space for you to mole, here is your money back.", GAME_NAME );
		}

		// Moving b/c of skill
		else
		{
			client_print( id, print_chat, "%s No space for you to mole.", GAME_NAME );
		}
	}
...
public _SHARED_Teleport( parm[] )
{
	...
	if ( vOrigin[2] == parm[2] )
	{
		...
		if ( iSpawnEnt > 0 )
		{
			...
			client_print( id, print_chat, "%s You are stuck, moving you back to spawn.", GAME_NAME );
		}

		// We can't move the user 
		else
		{
			client_print( id, print_chat, "%s You are stuck, moving you back to spawn later.", GAME_NAME );
		}
	}
}
