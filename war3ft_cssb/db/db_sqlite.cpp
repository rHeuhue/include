...
{
	"CREATE TABLE `cssb_player` ( `player_id` INTEGER PRIMARY KEY AUTOINCREMENT, `player_steamid` varchar(25) NOT NULL default '', `player_ip` varchar(20) NOT NULL default '', `player_name` varchar(35) NOT NULL default '', `time` timestamp(14) NOT NULL DEFAULT CURRENT_TIMESTAMP );",
	"CREATE TABLE `cssb_player_race` ( `player_id` INT NOT NULL default '0', `race_id` tinyint(4) NOT NULL default '0', `race_xp` INT default NULL, PRIMARY KEY  (`player_id`,`race_id`) );",
	"CREATE TABLE `cssb_player_skill` ( `player_id` INT NOT NULL default '0', `skill_id` tinyint(4) NOT NULL default '0', `skill_level` tinyint(4) NOT NULL default '0', PRIMARY KEY  (`player_id`,`skill_id`) );",
	//Для talisman +
	"CREATE TABLE `cssb_player_talisman` ( `player_id` INT NOT NULL default '0', `talisman_id` tinyint(4) NOT NULL default '0',PRIMARY KEY  (`player_id`,`talisman_id`) );",
	"CREATE TABLE `cssb_num_talisman` ( `player_id` INT NOT NULL default '0', `talisman_num` tinyint(4) NOT NULL default '0',PRIMARY KEY  (`player_id`,`talisman_num`) );"
};
...
new const szTableNames[TOTAL_SQLITE_TABLES][] = 
{
	...
	"cssb_player_race",
	"cssb_player_skill",
	"cssb_player_talisman",	//Для talisman +
	"cssb_num_talisman"		//Для talisman +
};
...
SQLITE_FetchUniqueID( idUser)
{
	...
	new szQuery[512];
	format( szQuery, 511, "SELECT `player_id` FROM `cssb_player` WHERE `%s` = '%s';", szKeyName, szKey );
	...
	if ( SQL_NumResults( query ) == 0 )
	{
		...
		// Insert this player!
		new szQuery[512];
		format( szQuery, 511, "INSERT INTO `cssb_player` ( `player_id` , `%s` , `time` ) VALUES ( NULL , '%s', julianday('now') );", szKeyName, szKey );
		new Handle:query = SQL_PrepareQuery( g_DBConn, szQuery );
...
SQLITE_Prune()
{
	new const szPruneQuery[SQLITE_TOTAL_PRUNE_QUERY][] = 
	{
		...
		"DELETE FROM cssb_player_skill WHERE player_id IN ( SELECT `player_id` FROM `cssb_player` WHERE ( (julianday(`time`) + %d) < julianday('now') ) );"
	};
...
SQLITE_UpdateTimestamp( idUser )
{
	...
	new szQuery[256];
	format( szQuery, 255, "UPDATE `cssb_player` SET time = julianday('now') WHERE ( `player_id` = '%d' )", g_iDBPlayerUniqueID[idUser] );
