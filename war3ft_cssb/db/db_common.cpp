...
public DB_DetermineType()
{
	...
	// See if we can save with SQLite if MYSQL didn't work
	if ( g_DBType == -1 )
	{
...
public DB_SaveXP( id, bThreaded )
{
	...
	new iRaceID = arrIntData[id][P_RACE];
...
// ¬озвращает значение по которому сохран€етс€ в базу данных (ник,IP,SteamID)
...
// ¬озвращает ключ имени таблицы по которой идет сохранение
...
public DB_GetAllXP(idUser,iSelectIdMenu )
{
	...
	switch( g_DBType )
	{
		case DB_MYSQLX:	MYSQLX_GetAllXP(idUser,iSelectIdMenu );
		case DB_SQLITE:	SQLITE_GetAllXP(idUser,iSelectIdMenu );
	}
...
DB_Prune()
{
	...
	if ( !get_pcvar_num( CVAR_wc3_save_pruning ) || !get_pcvar_num( CVAR_wc3_days_before_delete ) )
		return;
