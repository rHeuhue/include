...
#define SH_BIGBADVOODOO_DURATION  3
		...
		if ( p_data_b[targetid][PB_ISCONNECTED] && get_user_team( targetid ) == iTeam )
		{
			...
			if ( get_distance(origin, targetorigin) < SH_HEALING_WAVE_RANGE )
			{
				...
				if ( get_user_health( targetid ) + 1 <= get_user_maxhealth( targetid ) )
				{
					...
					//SHARED_Glow( targetid, 200, 200, 0, 0 );
				}
			}
		}
	}
...
public SH_PlaceSerpentWard( id )
{
	...
	if ( p_data_b[id][PB_HEXED] )
	{
		WC3_StatusText( id, 0, "Hex blocks your abilities.");
	}
	...
	else if ( is_user_alive( id ) && SM_GetSkillLevel( id, SKILL_SERPENTWARD ) > 0 && p_data[id][P_SERPENTCOUNT] > 0 )
	{
		...
		if ( SH_CanPlaceWard( id ) )
		{
			...
			WC3_StatusText( id, 0, "Serpent Ward Placed^nWards Left: %d", p_data[id][P_SERPENTCOUNT] );
		}
		else
		{
			WC3_StatusText( id, 0, "You can not place a serpent ward here." );
		}
	}
}
...
public _SH_DrawSerpentWard( parm[5] )
{
	...
	if ( bWardBombCheck )
	{
		...
		{
			client_print_color( id, print_team_default, "^4%s ^3Ward ^1removed, you can not have them near ^3bombs ^1or ^4hostages^1.", GAME_NAME );
...
public _SH_ResetBigBadAttacker( id )
{
	...
	client_print_color( id, print_team_default, "^4%s ^1You can not damage players with ^3Big Bad Voodoo^1.", GAME_NAME );
}