	...
	{		
		...
		if ( ent < 0 )
		{
			...
			if ( ent == -2 )
			{
				...
				WC3_StatusText( id, 0, "Vegeance failed because of enemy immunity." );
			}

			// No spawn found
			else
			{
				WC3_StatusText( id, 0, "No space for your vegeance." );
			}
		}
...
WA_Blink( id )
{
	...
	if ( iSkillLevel > 0 )
	{
		...
		if ( random_float( 0.0, 1.0 ) <= p_blink[iSkillLevel-1] )
		{
			...
			client_print_color( id, print_team_default, "^4%s ^1You will now be ^3immune ^1to ^4all ultimates^1.", GAME_NAME );
		}
	}
