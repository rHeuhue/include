...
SHARED_HasSmokeGrenade( id )
{
	...
	while ( g_PlayerWeapons[id][i] && !bNadeFound )
	{
		if ( SHARED_IsSmokeGrenade( g_PlayerWeapons[id][i] ) )
		{
			bNadeFound = true;
			break;
		}
...
public _SHARED_Spawn( id )
{
	...
	if ( g_EndRound )
	{
		...
		{
			client_print_color( id, print_team_default, "^4%s ^3No respawn because the round is over, here is your money back.", GAME_NAME );
...
public _SHARED_Mole( parm[2] )
{
	...
	else
	{
		...
		if ( parm[1] == 2 )
		{
			...
			client_print_color( id, print_team_default, "^4%s ^3No space for you to mole, here is your money back.", GAME_NAME );
		}

		// Moving b/c of skill
		else
		{
			client_print_color( id, print_team_default, "^4%s ^3No space for you to mole.", GAME_NAME );
		}
	}
...
public _SHARED_Teleport( parm[] )
{
	...
	if ( vOrigin[2] == parm[2] )
	{
		...
		if ( iSpawnEnt > 0 )
		{
			...
			client_print_color( id, print_team_default, "^4%s ^1You are stuck, moving you back to spawn.", GAME_NAME );
		}

		// We can't move the user 
		else
		{
			client_print_color( id, print_team_default, "^4%s ^1You are stuck, moving you back to spawn later.", GAME_NAME );
		}
	}
}
