...
public _CL_ULT_LocustEffect( parm[] )
{
	...
	{
		//client_print( iAttacker, print_chat, "%s Victim is hidden for Locust Swarm, we finding new target.", GAME_NAME );
		...
		client_print_color( iAttacker, print_team_default, "^4%s ^1Victim is hidden for ^3Locust Swarm^1.", GAME_NAME );
	...
	else
	{
		...
		client_print_color( iAttacker, print_team_default, "^4%s ^3Locust Swarm ^1hit ^4%s ^1with ^3%d Damage^1.", GAME_NAME, szName, iDamage );
	}
}
