...
{
	"CREATE TABLE IF NOT EXISTS `wc3_player` ( `player_id` int(8) unsigned NOT NULL auto_increment, `player_steamid` varchar(25) NOT NULL default '', `player_ip` varchar(20) NOT NULL default '', `player_name` varchar(35) NOT NULL default '', `time` timestamp(6) NOT NULL, PRIMARY KEY  (`player_id`), KEY `player_name` (`player_name`), KEY `player_ip` (`player_ip`), KEY `player_steamid` (`player_steamid`) ) Engine=MyISAM;",
...
new const szTables[TOTAL_TABLES][] = 
{
	"CREATE TABLE IF NOT EXISTS `wc3_player` ( `player_id` int(8) unsigned NOT NULL auto_increment, `player_steamid` varchar(25) NOT NULL default '', `player_ip` varchar(20) NOT NULL default '', `player_name` varchar(35) NOT NULL default '', `time` timestamp(6) NOT NULL, PRIMARY KEY  (`player_id`), KEY `player_name` (`player_name`), KEY `player_ip` (`player_ip`), KEY `player_steamid` (`player_steamid`) ) TYPE=MyISAM;",
...
MYSQLX_Save( id )
{
	...
	if ( !SQL_Execute( query ) )
	{
		client_print_color( id, print_team_default, "^4%s ^3Unable to save your XP, please contact a server administrator.", GAME_NAME );
...
MYSQLX_GetAllXP( id )
{
	...
	if ( iUniqueID <= 0 )
	{
		client_print_color( id, print_team_default, "^4%s ^3Unable to retreive your XP, please changerace later.", GAME_NAME );
	...
	if ( !SQL_Execute( query ) )
	{
		client_print_color( id, print_team_default, "^4%s ^3Unable to retrieve your XP, please contact a server administrator.", GAME_NAME );
...
MYSQLX_SetDataForRace( id )
{
	...
	if ( !SQL_Execute( query ) )
	{
		client_print_color( id, print_team_default, "^4%s ^3Unable to retrieve XP, please contact a server administrator.", GAME_NAME );
...
MYSQLX_UpdateWebTable()
{
	...
	formatex ( szQuery, 255, "SELECT `config_value` FROM `wc3_config` WHERE `config_id` = 'version' AND `config_value` = '%s';", PLUGIN_VERSION );
	query = SQL_PrepareQuery( g_DBConn, szQuery );
	...
	else
	{
		...
		formatex ( szQuery, 1023, "REPLACE INTO `wc3_config` ( `config_id` , `config_value` ) VALUES ( 'version', '%s' );", PLUGIN_VERSION );
		query = SQL_PrepareQuery( g_DBConn, szQuery );
		...
		for ( iLang = 0; iLang < iTotalLanguages; iLang++ )
		{
			...
			if ( lang_exists( lang ) )
			{
				...
				for ( i = 1; i <= MAX_RACES; i++ )
				{
					...
					formatex( szQuery, 1023, "REPLACE INTO `wc3_web_race` ( `race_id` , `race_lang` , `race_name`, `race_description` ) VALUES ( '%d', '%s', '%s', '' );", i, lang, Lang_RaceName[i] );
				...
				{
					formatex(szDescription, 511, "%s", Lang_SkillUltimateDatabase[i][SKILL_DESCRIPTION]);
					DB_FormatString( szDescription, 511 );

					formatex( szQuery, 1023, "REPLACE INTO `wc3_web_skill` ( `skill_id` , `skill_lang` , `skill_name`, `skill_description`, `skill_type`, `skill_owner` ) VALUES ( '%d', '%s', '%s', '%s', '%d', '%d' );", i, lang, Lang_SkillUltimateDatabase[i][SKILL_NAME], Lang_SkillUltimateDatabase[i][SKILL_DESCRIPTION], g_SkillType[i], g_SkillOwner[i] );
