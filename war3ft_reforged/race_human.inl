...
HU_ULT_Blink( id )
{
	...
	{
		client_print( id, print_center, "You can not use your ultimate while planting the bomb." );
		return;
	}
	...
	{
		WC3_StatusText( id, 0, "You can not blink when you are stunned." );
	...
	if ( iLosesCharge > 0 )
	{
		...
		WC3_StatusText( id, 0, "Teleport failed because enemy has immunity.");
	...
	{
		WC3_StatusText( id, 0, "Teleport failed because of invalid location." );
...
HU_ULT_BlinkProtection( id, vOrigin[3] )
{
	...
	if ( bSlay )
	{
		client_print_color( id, print_team_default, "^4%s ^3You have been slain for teleporting into a restricted area.", GAME_NAME);
		user_kill( id, 1 );
	}
