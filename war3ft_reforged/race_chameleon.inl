...
public CHAM_Randomize()
{
	if ( get_pcvar_num( CVAR_wc3_cham_random ) )
	{
		...
		for ( i = 0; i < iNumPlayers; i++ )
		{
			...
			if ( p_data[id][P_RACE] == RACE_CHAMELEON && !Polymorph_Player[id] )
			{
...
public LoadPolymorph(id) 
{
	new Name[32];
	get_user_name(id, Name, 31);
	
	new Line[128], IsPlayer = false, Arg1[32], Arg2[32], Arg3[32], Arg4[32], Arg5[32], Arg6[32];
	
	new FileOpen = fopen(PolymorphFile, "rt")
	while(!feof(FileOpen)) 
	{
		fgets(FileOpen, Line, 127)
		trim(Line)
		
		parse(Line, Arg1, 31, Arg2, 31, Arg3, 31, Arg4, 31, Arg5, 31, Arg6, 31)
		
		if (equali(Arg1, Name)) 
		{
			Polymorph_Player[id] = true; 
			
			Polymorph_Skill_1[id] = str_to_num(Arg2); 
			Polymorph_Skill_2[id] = str_to_num(Arg3); 
			Polymorph_Skill_3[id] = str_to_num(Arg4); 
			Polymorph_Ultimate[id] = str_to_num(Arg5); 
			Polymorph_Passive[id] = str_to_num(Arg6); 
