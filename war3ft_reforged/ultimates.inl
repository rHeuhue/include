...
ULT_IconHandler( id )
{
	...
	else
	{
		...
		WC3_StatusText( id, TXT_ULTIMATE, "Ultimate Ready !" );
...
ULT_Icon( id, flag )
{
	...
	if ( p_data[id][P_RACE] == RACE_CHAMELEON )
	{
		...
		{
		  iChamUltimate = Polymorph_Ultimate[id];
		}
		else 
		{
		  iChamUltimate = g_ChamSkills[3];
		}
	...
	if ( iRaceID )
	{
		...
		switch ( iRaceID )
		{
			...
			case 9: r=0,	g=100,	b=200;		// Crystal
			case 10: r=135,	g=135,	b=135;		// Druid
		}
...
ULT_RemoveCharge( id, iFuncID )
{
	...
	client_print_color( id, print_team_default, "^4%s ^3You have blocked an enemy ultimate.", GAME_NAME );
...
ULT_Blocked( id )
{
	...
	client_print_color( id, print_team_default, "^4%s ^3Your ultimate has been blocked.", GAME_NAME );
...
ULT_Reset( id )
{
	...
	// Reset Crow Form
	if ( task_exists( TASK_CROWFORM + id ) )
	{
		remove_task( TASK_CROWFORM + id );
